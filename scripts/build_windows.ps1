<#
.SYNOPSIS
  Heimdall — build a signed Windows installer and raw zip on an unattended CI runner.

.DESCRIPTION
  Designed to run end-to-end with no interactive agent: code-signing secrets
  arrive through environment variables, the script decodes the .pfx into a
  ...
  Outputs:
    $OUTPUT_DIR\heimdall-setup-<version>.exe   signed installer
    $OUTPUT_DIR\heimdall-<version>-windows.zip raw Release folder, zipped
